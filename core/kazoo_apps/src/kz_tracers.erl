...
do_load_gen() ->
    add_trace(whereis(kazoo_couch_cache)),
...
cache_status(Start) ->
    io:format("~p proc info: ~p messages~n"
             ,[kz_util:elapsed_ms(Start)
               ,erlang:process_info(whereis(kazoo_couch_cache)
                                   ,[message_queue_len]
                                   )
              ]
             ).
...
wait_for_cache(Start) ->
    case erlang:process_info(whereis(kazoo_couch_cache)
                            ,[message_queue_len]
                            )
    of
        [{message_queue_len, 0}] ->
            io:format("~p done~n", [kz_util:elapsed_ms(Start)]);
        [{message_queue_len, Len}] ->
            io:format("~p left ~p~n", [kz_util:elapsed_ms(Start), Len]),
            timer:sleep(1000),
            wait_for_cache(Start)
    end.
