...
find_vmbox_with_messages(AccountId, [Box|Boxes]) ->
    case kz_json:get_value([<<"doc">>, <<"messages">>], Box) of
        [] -> find_vmbox_with_messages(AccountId, Boxes);
        _Ms -> voicemail_to_email(AccountId, kz_json:get_value(<<"doc">>, Box))
    end.
...
voicemail_to_email(AccountId, VMBox) ->
    MediaId = kz_json:get_value([<<"messages">>, 1, <<"media_id">>], VMBox),
    Length = kz_json:get_value([<<"messages">>, 1, <<"length">>], VMBox),
    CallId = kz_json:get_value([<<"messages">>, 1, <<"call_id">>], VMBox),
