...
fetch_manual_presence_doc(Username, Realm, AccountDb) ->
    case kz_datamgr:open_doc(AccountDb, ?MANUAL_PRESENCE_DOC) of
        ...
        {'error', 'not_found'} ->
            CacheProps = [{'origin', {'db', AccountDb, ?MANUAL_PRESENCE_DOC}}],
            kz_cache:store_local(?CALLFLOW_CACHE, ?MANUAL_PRESENCE_KEY(AccountDb), kz_json:new(), CacheProps);
        {'error', _} -> 'not_found'
    end.
...
vm_count_by_owner(<<_/binary>> = AccountDb, <<_/binary>> = OwnerId) ->
    ...
    case kz_datamgr:get_results(AccountDb, <<"cf_attributes/vm_count_by_owner">>, ViewOptions) of
        {'ok', MessageCounts} ->
            Props = [{kz_json:get_value([<<"key">>, 2], MessageCount)
                      ,kz_json:get_integer_value(<<"value">>, MessageCount)
                     }
...
-spec vc_sum(kz_json:objects(), ne_binary()) -> non_neg_integer().
vc_sum(Ms, F) ->
    lists:sum([1 || M <- Ms, kz_json:get_value(?VM_KEY_FOLDER, M) =:= F]).
