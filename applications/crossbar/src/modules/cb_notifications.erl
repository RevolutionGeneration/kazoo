...
read_system(Context, Id) ->
    crossbar_doc:load(Id, cb_context:set_account_db(Context, ?KZ_CONFIG_DB)).
...
read_system_attachment(Context, DocId, Name) ->
    crossbar_doc:load_attachment(DocId, Name, cb_context:set_account_db(Context, ?KZ_CONFIG_DB)).
...
update_template(Context, Id, FileJObj) ->
    ...
    Opts = [{'content_type', kz_util:to_list(CT)}], % Temporary until couchbeam update
